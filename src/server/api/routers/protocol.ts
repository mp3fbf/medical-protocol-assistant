...
export const protocolRouter = router({
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      console.log('[TRPC /protocol.create] Received input on server:', JSON.stringify(input, null, 2));

      const { title, condition } = input;
      const userId = ctx.session.user.id;

      if (!title || !condition) {
        console.error('[TRPC /protocol.create] Server received invalid input (missing title or condition):', input);
        throw new TRPCError({
          code: "BAD_REQUEST",
      ...
      try {
        ...
        console.log(`[TRPC /protocol.create] Successfully created protocol in DB: ${protocol.id} - ${protocol.title}`);
        ...
        console.error(`[TRPC /protocol.create] Error during protocol creation in DB:`, error);
        throw new TRPCError({
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      console.log(`[TRPC /protocol.getById] Received input on server: { protocolId: "${input.protocolId}" }`);
      ...
      if (!protocol) {
        console.warn(`[TRPC /protocol.getById] Protocol with ID '${input.protocolId}' not found.`);
        throw new TRPCError({
          code: "NOT_FOUND",
          message: `Protocolo com ID '${input.protocolId}' não encontrado.`,
        });
      }
      console.log(`[TRPC /protocol.getById] Successfully fetched protocol: ${protocol.id}`);
      return protocol;
    }),
