...
// import { randomUUID } from "crypto"; // Prisma now handles ID generation with @default(cuid())
import { logAuditEvent, AuditActions } from "@/lib/audit"; // Import audit logging
...
export const protocolRouter = router({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const protocol = await ctx.db.protocol.create({
        data: {
          ...
          // updatedAt will be handled by @updatedAt
          ProtocolVersion: {
            create: [
              {
                // id will be generated by cuid()
      ...
      // Log audit event for protocol creation
      await logAuditEvent(ctx.db, {
        ...
        // ipAddress: ctx.req?.headers?.['x-forwarded-for'] as string | undefined // If needed
      });
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Update the protocol's own updatedAt timestamp
      await ctx.db.protocol.update({
        where: { id: protocolId },
        data: { updatedAt: new Date() },
      });

      // Log audit event for protocol version creation
      await logAuditEvent(ctx.db, {
        userId: userId,
