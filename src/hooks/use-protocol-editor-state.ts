...
type ProtocolGetByIdOutput = inferProcedureOutput<AppRouter['protocol']['getById']>;

const generateMockProtocolData = (protocolIdTitlePart: string): ProtocolFullContent => {
...
const isValidCuid = (id: string): boolean => id.startsWith('c') && id.length >= 24 && id.length <= 30;
...
export function useProtocolEditorState(initialProtocolId?: string) {
  ...
  const protocolQuery = trpc.protocol.getById.useQuery(
    ...
    {
      enabled: !!initialProtocolId && initialProtocolId !== "new" && isValidCuid(initialProtocolId),
  ...
  useEffect(() => {
    if (protocolQuery.isSuccess && protocolQuery.data) {
      ...
      const protocolContent = (data.ProtocolVersion?.[0]?.content as unknown as ProtocolFullContent) || generateMockProtocolData(data.title);
      setState((s) => ({
        ...s,
        protocolId: data.id,
        protocolTitle: data.title,
        protocolData: protocolContent,
        flowchartData: (data.ProtocolVersion?.[0]?.flowchart as unknown as FlowchartDefinition) || initialEmptyFlowchart,
        isLoading: false,
        error: null,
      }));
    }
  ...
  useEffect(() => {
    if (protocolQuery.isError && protocolQuery.error) {
      ...
      console.error('[useProtocolEditorState] trpc.protocol.getById onError (useEffect): ', err);
      setState((s) => ({
        ...s,
  ...
  const fetchProtocolData = useCallback(async (id: string) => {
    console.log('[useProtocolEditorState] fetchProtocolData called with id:', id);
      ...
      try {
        ...
        if (data) {
           ...
           setState((s) => ({
            ...
            protocolId: data.id,
            protocolTitle: data.title,
            protocolData: (data.ProtocolVersion?.[0]?.content as unknown as ProtocolFullContent) || generateMockProtocolData(data.title),
            flowchartData: (data.ProtocolVersion?.[0]?.flowchart as unknown as FlowchartDefinition) || initialEmptyFlowchart,
            isLoading: false,
            error: null,
          }));
        ...
        setState((s) => ({ ...s, error: `Falha ao recarregar: ${(err as Error).message}`, isLoading: false }));
      }
    } else {
       console.warn('[useProtocolEditorState] fetchProtocolData: Invalid ID, not fetching:', id);
  ...
  useEffect(() => {
    console.log('[useProtocolEditorState] useEffect triggered. initialProtocolId:', initialProtocolId);
    if (initialProtocolId && initialProtocolId !== "new" && isValidCuid(initialProtocolId)) {
      // Data fetching is handled by useQuery, isLoading will be true initially
      setState(s => ({...s, isLoading: protocolQuery.isLoading, error: protocolQuery.error?.message || null}));
    } else if (initialProtocolId === "new" || !isValidCuid(initialProtocolId || "")) {
      // Setup for a new protocol if ID is "new" or invalid
      console.log('[useProtocolEditorState] Setting up for a new protocol or invalid ID.');
      setState((s) => ({
        ...s,
        isLoading: false,
        protocolId: "new", 
        ...
        console.warn('[useProtocolEditorState] No initialProtocolId and not loading, setting to new protocol state.');
         setState((s) => ({
            ...s,
            isLoading: false,
            protocolId: "new", 
      ...
      return {
        ...
        protocolData: {
          ...
          [key]: {
            ...
            ...(s.protocolData[key] || { sectionNumber, title: SECTION_DEFINITIONS.find(sd => sd.sectionNumber === sectionNumber)?.titlePT || `Seção ${sectionNumber}` }),
            content: newContent,
          },
        },
      };
  ...
  const saveProtocol = async () => {
    if (!state.protocolData || !state.protocolId || state.protocolId === "new") {
        console.error("[useProtocolEditorState] Cannot save: protocol data or ID is missing/invalid.");
        ...
        alert("Funcionalidade de salvar ainda não implementada completamente ou ID de protocolo inválido.");
        return;
    }
    console.log("[useProtocolEditorState] Simulating save for protocol:", state.protocolId, state.protocolData);
